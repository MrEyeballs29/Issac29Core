...
public class ExtendedOreFeature extends Feature<ExtendedOreFeatureConfig> {

	public ExtendedOreFeature(Function<Dynamic<?>, ? extends ExtendedOreFeatureConfig> configFactoryIn) {
		super(configFactoryIn);
	}

	@Override
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand,
			BlockPos pos, ExtendedOreFeatureConfig config) {
		float f = rand.nextFloat() * (float)Math.PI;
		float f1 = config.size / 8.0F;
		int i = MathHelper.ceil((config.size / 16.0F * 2.0F + 1.0F) / 2.0F);
		double d0 = pos.getX() + MathHelper.sin(f) * f1;
		double d1 = pos.getX() - MathHelper.sin(f) * f1;
		double d2 = pos.getZ() + MathHelper.cos(f) * f1;
		double d3 = pos.getZ() - MathHelper.cos(f) * f1;
	      ...
	      for(int i3 = 0; i3 < config.size; ++i3) {
	         ...
	         if (!(d11 < 0.0D)) {
	            ...
	            for(int l1 = k; l1 <= i1; ++l1) {
	               double d8 = (l1 + 0.5D - d1) / d11;
	               if (d8 * d8 < 1.0D) {
	                  for(int i2 = k3; i2 <= j1; ++i2) {
	                     double d9 = (i2 + 0.5D - d3) / d11;
	                     if (d8 * d8 + d9 * d9 < 1.0D) {
	                        for(int j2 = l; j2 <= k1; ++j2) {
	                           double d10 = (j2 + 0.5D - d5) / d11;
	                           if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
	                              ...
	                              if (!bitset.get(k2)) {
	                                 ...
	                                 if (config.target.equals(worldIn.getBlockState(blockpos$mutable))) {
