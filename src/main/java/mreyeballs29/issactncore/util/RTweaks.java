...
public class RTweaks implements IShapedRecipe {

	private NonNullList<Ingredient> recipeItems = NonNullList.from(Ingredient.EMPTY, new Ingredient[] {new OreIngredient("plankWood"), new OreIngredient("plankWood")});
	private int height;
	private int width;
	...
	public RTweaks(int height, int width, ItemStack output, String group, Ingredient... ingredients) {
		...
		if (ingredients.length <= this.height*this.width) {
			...
			throw new IllegalArgumentException("Too many items in the list");
		}
		this.group = group;
		this.stack = output;
	}
